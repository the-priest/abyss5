...
# Then creates:
#   - the `abyss` command on your PATH
#   - an app-launcher entry with icon (Linux only)
# Also fetches public-domain music tracks the first time it's run.
...
# 1. core tools
step 1 7 "checking core tools (git, python3, pip)"
ensure_tool git git
...
# 2. pygame
step 2 7 "checking pygame"
ensure_pygame

# 3. clone or pull
step 3 7 "fetching $GAME_NAME repo"
...
# 4. wrapper command
step 4 7 "installing '$WRAPPER_CMD' command"
WRAPPER_BODY='#!/bin/sh
...
step 5 7 "installing app launcher entry"
...
# 6. optional music
step 6 7 "fetching public-domain music (first run only)"
if [ -f "$INSTALL_DIR/fetch_music.py" ]; then
    python3 "$INSTALL_DIR/fetch_music.py" || warn "music fetch failed — you can re-run later"
    ok "music done"
else
    warn "fetch_music.py not in repo, skipping"
fi
